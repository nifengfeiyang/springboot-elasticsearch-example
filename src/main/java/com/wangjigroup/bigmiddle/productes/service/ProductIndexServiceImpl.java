...
public class ProductIndexServiceImpl implements ProductIndexService {
    ...
    @Override
    public int updateProductIndex(CancelTheSelectionTopic topic) throws IOException {
        return updateSkuInfos(topic.getProductId().toString(), topic.getProductSkuId(), topic.getShopId(), null, new Date());
    }

    @Override
    public int updateProductIndex(ReSelectProductTopic topic) throws IOException {
        return updateSkuInfos(topic.getProductId().toString(), topic.getProductSkuId(), topic.getShopId(), new Date(), null);
    }

    @Override
    public int updateProductIndex(SelectProductTopic topic) throws IOException {
        return updateSkuInfos(topic.getProductId().toString(), topic.getProviderItemId(), topic.getShopId(), new Date(), null);
    }

    private int updateSkuInfos(String productId, Long productSkuId, Long shopId, Date onlineTime, Date offlineTime) throws IOException {
        ...
        if (getResponse.isExists()) {
            ...
            throw new ServiceException(productId + ": index not exists!!!");
        }
        return 1;
    }
